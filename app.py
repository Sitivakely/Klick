...
def load_high_tech_css():
    """Injecte le style visuel futuriste."""
    st.markdown("""
        <style>
            :root {
                --main-bg: #0d1117;
...
# --- 3. CONNEXION GOOGLE SHEETS SÉCURISÉE (sans fichier JSON) ---
...
def init_gspread():
    ...
    try:
        ...
        if "gcp_service_account" not in st.secrets:
            st.error("ERREUR : Section [gcp_service_account] absente dans les secrets Streamlit.")
            st.stop()

        creds_dict = dict(st.secrets["gcp_service_account"])

        # ⚠️ Ne pas toucher à la clé privée si elle est déjà bien formatée (avec triple quotes)
        ...
        st.error(f"ERREUR : Feuille '{SPREADSHEET_NAME}' introuvable. Vérifiez le nom exact et le partage.")
        st.stop()
    except Exception as e:
        st.error(f"Erreur lors de l'initialisation de Google Sheets : {e}")
        st.stop()


def _ensure_headers(sheets):
    """Vérifie et corrige les en-têtes de chaque feuille si nécessaire."""
    headers = {
...
def fetch_data(sheet_name):
    """Récupère toutes les lignes d'une feuille."""
    try:
...
def main_app():
    ...
    # 👉 Ici vient tout le reste de votre interface, chronomètre, gestion tâches, reporting, etc.
    st.info("✅ Connexion réussie à Google Sheets via Streamlit Secrets. Votre app fonctionne !")
