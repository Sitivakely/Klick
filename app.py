...
def init_gspread():
    ...
    try:
        # Tente de charger les identifiants du fichier service_account.json
        scope = ['https://spreadsheets.google.com/feeds',
                 'https://www.googleapis.com/auth/drive']
        
        # Vérification si le fichier de compte de service existe
        if not os.path.exists("service_account.json"):
            st.error("ERREUR : Fichier 'service_account.json' manquant.")
            st.stop()
        ...
        # Charger les feuilles (Worksheets)
        sheets = {
        ...
        st.error(f"Erreur lors de l'initialisation de Google Sheets. Assurez-vous que l'API est activée et que le fichier `service_account.json` est valide. Détail: {e}")
        st.stop()
